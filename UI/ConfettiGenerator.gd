...
var image_file = "res://Assets/Backgrounds/Animations/confetti_tiles.png"
var current_confetti_type = ""
...
func start(direction:Vector2, emission_shape:int, amount:int):
	if current_confetti_type in confetti_types:
		emitter.texture = confetti_tiles[current_confetti_type]
		emitter.direction = direction
		emitter.emission_shape = emission_shape
		emitter.amount = amount
		$CPUParticles2D.emitting = true
	else:
		print("Must set confetti type first")
