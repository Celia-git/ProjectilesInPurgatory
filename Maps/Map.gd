...
func _ready():
	...
	background_textures = texture_script.get_profile_backgrounds()
...
func _add_main_scene():
	...
	if is_instance_of(main, InteriorScenes):			
		main.active_scene_idx = player.get_current_subgame()	
		main.ext_map_idx =player.get_previous_map_idx()
		pixel_frame_visible = !(main.is_in_group("Cinematic"))
		if !main.update_ui_color.is_connected(_set_pixel_frame_color):
			main.update_ui_color.connect(_set_pixel_frame_color)
			
		if !main.tree_exiting.is_connected(_save_scene_data.bind(main.active_scene_idx)):
			main.tree_exiting.connect(_save_scene_data.bind(main.active_scene_idx))
	...
	###### Removed: Causes set_new_active_scene to be called twice
	#if is_instance_of(main, InteriorScenes):
...
# Called by guestbook
func _load_game_file(username):
	
	# Load player data
	Globals.player = load_script.load_player(username)
	player = Globals.player
	Globals.game_states = load_script.load_game_data(username)
...
func _save_game(terminate):
	...
	if main != null:
		...
		player.set_active_map_type(["Interior", "Exterior"][int(is_instance_of(main, ExteriorScenes))])
